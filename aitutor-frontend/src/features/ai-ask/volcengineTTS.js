...
export async function volcengineTTS(params) {
  ...
  const body = {
    ...
    audio: {
      ...
      encoding: "mp3",
      speed_ratio: speed,
      loudness_ratio: volume, // 火山引擎使用 loudness_ratio
      emotion: emotion,
    },
    request: {
  ...
  try {
    ...
    if (data.code === 3000 && data.data) {
      return {
        audio: data.data, // base64 编码的音频数据
        success: true,
      };
    } else {
...
export function mapStyleToEmotion(style) {
  const styleMap = {
    ...
    angry: "angry",
    fear: "fearful",
    surprised: "neutral", // 无对应情绪时回退到 neutral
  };
...
export async function volcengineCompatibleTTS(message, speakerX, speakerY, style) {
  ...
  const index = Math.floor(speakerX * voiceTypes.length);
  const voice_type = voiceTypes[index] || voiceTypes[0];

  // 根据 speakerY 调整语速 (0.5-2.0)
